...
resource "aws_instance" "docker-machine-managers" {
  ...
  tags = {
    Name = "adem-Docker-Swarm-Manager-${count.index + 1}"
...
resource "aws_instance" "docker-machine-workers" {
  ...
  tags = {
    Name = "adem-Docker-Swarm-Worker-${count.index + 1}"
